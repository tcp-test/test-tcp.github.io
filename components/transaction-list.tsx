...
export default function TransactionList() {  
  ...
  query RecordQuery {
    c2Corders(
      where: {seller: "${address?.toLocaleLowerCase()}"}
      orderBy: createTime
      orderDirection: desc
    ) {
      price
                ...
                <article key={index} className="h-full flex flex-col bg-white p-6 shadow-xl" data-aos="fade-up" data-aos-anchor="[data-aos-id-testimonials]">
                  <header className=''>
                    <div className='flex items-center align-middle space-x-1 font-bold'>
                      {
                        item.c2CAdvertised.orderType == 1 && <span className='text-green-500'>购买</span>
                      }
                      {
                        item.c2CAdvertised.orderType == 0 && <span className='text-red-500'>出售</span>
                      }
                  ...
                  <div className="flex text-sm mt-2">
                      ...
                      <p className="">{truncateAddress(item.owner)}</p>
                    </div>
                  </div>
                </article>
                )
